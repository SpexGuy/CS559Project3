...
vec4 ads() {
	vec3 color = 0.5 + 0.5*vec3(snoise(GVPosition), snoise(GVPosition), snoise(GVPosition));
	vec3 n = normalize(GNormal);
	vec3 return_color = vec3(0.0f);
	if (!gl_FrontFacing)
	n = -n;
	for(int i = 0; i < 5; i++) {
		vec3 s = normalize(light_position[i] - GVPosition);
	...
	return vec4(return_color, 0.8);
}

void main(){
	//if (snoise(GPPosition) > 0) discard;
